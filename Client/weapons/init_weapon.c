...
** Last update Wed Oct  4 23:36:17 2017 Marc PEREZ
*/
...
t_weapon	*init_weapon(sfVector2f pos, int id)
{
  ...
  weapon->start = pos;
  weapon->is_hand = 0;
  weapon->sprite = init_sprite(AK47, vector2f(pos.x, pos.y),
			       vector2f(0.25, 0.25));
  weapon->damages = 10;
  ...
  if (id == 1 && (weapon->damages = AUG_DMG))
    weapon->sprite = init_sprite(AUG, vector2f(pos.x, pos.y),
				 vector2f(0.25, 0.25));
  if (id == 2 && (weapon->damages = FAMAS_DMG))
    weapon->sprite = init_sprite(FAMAS, vector2f(pos.x, pos.y),
				 vector2f(0.25, 0.25));
  if (id == 3 && (weapon->damages = M416_DMG))
    weapon->sprite = init_sprite(M416, vector2f(pos.x, pos.y),
				 vector2f(0.25, 0.25));
  if (id == 4 && (weapon->damages = MK14_DMG))
    weapon->sprite = init_sprite(MK14, vector2f(pos.x, pos.y),
				 vector2f(0.25, 0.25));
  return (weapon);
}
